...
def train_xgboost(X, y):
    """
    训练 XGBoost 模型，返回评估指标、特征重要性和绘图数据
    """
    ...
    # 特征重要性（使用内置的 feature_importances_）
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] != 'train':
        print(json.dumps({"status": "error", "message": "Usage: python ds02.py train"}))
        sys.exit(1)
    ...
    try:
        ...
        metrics, importance, plot_data = train_xgboost(X, y)
